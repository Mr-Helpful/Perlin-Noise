...
export function Levels(perlin, controls, defaults, defaultLevel) {
  this.perlin = perlin
  ...
  this.x = d3.scaleLinear().rangeRound([0, this.width]).domain([0, 8])
    ...
    c.data(this.points, d => d[0]).join(
      enter =>
        enter
          ...
          .call(enter => {
            return enter.transition().duration(T).style('opacity', 1)
          })
          .call(this.drag)
      ...
      exit =>
        exit
          .each(d => perlin.delLevel(d))
          .call(exit => {
            return exit.transition().duration(T).style('opacity', 0).remove()
          })
    )
  ...
  this.setControls = function (d) {
    let [[f, A, s], _] = perlin.fetchLevel(2 ** d[0])
    controls.select('#freq').html(f.toExponential(3))
    controls.select('#ampl').html(A.toExponential(3))
    controls.select('#seed').html(s.toExponential(3))
    controls.select('#delete').on('click', this.pointOps.rem.bind(this, d))
    controls
      .select('#reset')
  ...
  function setDefault(d, i) {
    ...
    scope.svg.selectAll('.graph.defaults').attr('r', 5)
    // select the clicked on circle
    scope.svg.selectAll('#default' + i).attr('r', 3.5)
    ...
    if (d.label != 'Custom') {
      ...
      scope.points = scope.points.map(v => [...v])
    }
...
function getPointOps(graph) {
  return {
    add(d) {
      d3.event.preventDefault()
      let [mx, my] = d3.mouse(this)
      d = [graph.x.invert(mx), graph.y.invert(my)]

      graph.points.push(d)
      graph.points.sort((a, b) => a[0] - b[0])
      graph.updateGraph()
    },
    rem(d) {
      d3.event.preventDefault()
      let i = graph.points.indexOf(d)
      ...
      graph.points = graph.flattenPoint(graph.points, i)
      graph.updateGraph(i)
    }
  }
}

function addPoint(elem, p) {
  const ps = this.points
  const i = searchFor(p, ps)
  assert(
    0 < i && i < ps.length - 1,
    'Attempting to place a point outside the anchors'
  )

  const T = 500 //ms

  let gLine = this.focus.select('.graph.line')
  let gDots = this.focus.selectAll('.graph.points')
...
function delPoint(elem, p) {
  const ps = this.points
  const i = searchFor(p, ps)
  assert(ps[i][0] == p[0], 'Attempting to remove a non-existant point')
  assert(0 < i && i < ps.length - 1, 'Attempting to remove an anchor point')
  ...
  elem.transition().duration(T).style('opacity', 0).remove()
...
/** Looks for the position a new point should go in,
 * or the position of the point itself, if it exists
 */
function searchFor(p, ps) {
  const x = p[0]
  for (let i = 0; i < ps.length; i++) {
    if (ps[i][0] >= x) return i
  }
  return ps.length
}
